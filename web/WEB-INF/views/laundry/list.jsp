...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1><i class="fas fa-tshirt mr-2"></i> Laundry Orders
                Management</h1>
            ...
            <c:if test="${not empty param.success}">
                <div class="alert alert-success alert-dismissible show fade">
                    <div class="alert-body">
                        ...
                        <c:choose>
                            <c:when test="${param.success == 'deleted'}">Order
                                deleted successfully!</c:when>
                            <c:when test="${param.success == 'updated'}">Order
                                updated successfully!</c:when>
                            <c:when
                                test="${param.success == 'status_updated'}">Status
                                updated successfully!</c:when>
                            <c:otherwise>Operation completed
                                successfully!</c:otherwise>
                        </c:choose>
                    </div>
                </div>
            </c:if>
            ...
            <c:if test="${not empty param.error}">
                <div class="alert alert-danger alert-dismissible show fade">
                    <div class="alert-body">
                        ...
                        <c:choose>
                            <c:when
                                test="${param.error == 'delete_failed'}">Failed
                                to delete order!</c:when>
                            <c:when
            ...
            <div class="card">
                <div
                    ...
                    <div class="card-header-action">
                        <a href="laundry-order?action=add"
                ...
                <div class="card-body p-0">
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select name="status" class="form-control">
                                        ...
                                        <option value="Pending" ${status ==
                                            'Pending' ? 'selected' :
                                            ''}>Pending</option>
                                        <option value="Washing" ${status ==
                                            'Washing' ? 'selected' :
                                            ''}>Washing</option>
                                        <option value="Ready" ${status ==
                                            'Ready' ? 'selected' :
                                            ''}>Ready</option>
                                        <option value="Delivered" ${status ==
                                            'Delivered' ? 'selected' :
                                            ''}>Delivered</option>
                                        <option value="Completed" ${status ==
                                            'Completed' ? 'selected' :
                                            ''}>Completed</option>
                                        <option value="Cancelled" ${status ==
                                            'Cancelled' ? 'selected' :
                                            ''}>Cancelled</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select name="sort" class="form-control">
                                        <option value="idDesc" ${sort ==
                                            'idDesc' ? 'selected' : ''}>Newest
                                            First</option>
                                        <option value="idAsc" ${sort == 'idAsc'
                                            ? 'selected' : ''}>Oldest
                                            First</option>
                                        <option value="pickupDesc" ${sort ==
                                            'pickupDesc' ? 'selected' :
                                            ''}>Pickup Date (Newest)</option>
                                        <option value="pickupAsc" ${sort ==
                                            'pickupAsc' ? 'selected' :
                                            ''}>Pickup Date (Oldest)</option>
                                        <option value="statusAsc" ${sort ==
                                            'statusAsc' ? 'selected' :
                                            ''}>Status (A-Z)</option>
                                        <option value="statusDesc" ${sort ==
                                            'statusDesc' ? 'selected' :
                                            ''}>Status (Z-A)</option>
                                    </select>
                                </div>
                    ...
                    <ul class="nav nav-tabs mb-3">
                        <li class="nav-item">
                            <a
                                class="nav-link ${empty view || view == 'Pending' ? 'active' : ''}"
                        ...
                        <li class="nav-item">
                            <a
                                class="nav-link ${view == 'Confirmed' ? 'active' : ''}"
                        ...
                        <li class="nav-item">
                            <a
                                class="nav-link ${view == 'Completed' ? 'active' : ''}"
                        ...
                        <li class="nav-item">
                            <a
                                class="nav-link ${view == 'Canceled' ? 'active' : ''}"
                                        ...
                                        <td class="text-right">
                                            <c:choose>
                                                <c:when
                                                    ...
                                                    <div class="btn-group">
                                                        <a
                                                            href="laundry-order?action=view&id=${order.laundryId}"
                                                        ...
                                                        <button type="button"
                                                            ...
                                                            onclick="confirmDelete(${order.laundryId})"
                                                            data-toggle="tooltip"
                                                            title="Cancel">
                                                            <i
                ...
                <c:if test="${totalPages > 1}">
                    <div class="card-footer text-right">
                        <nav class="d-inline-block">
                            <ul class="pagination mb-0">
                                <li
                                    ...
                                    <a class="page-link"
                                        href="laundry-order?action=list&page=${currentPage - 1}&search=${search}&status=${status}&view=${view}&sort=${sort}">
                                ...
                                <c:forEach begin="1" end="${totalPages}"
                                    ...
                                    <c:if
                                        ...
                                        <li
                                            ...
                                            <a class="page-link"
                                                href="laundry-order?action=list&page=${i}&search=${search}&status=${status}&view=${view}&sort=${sort}">
                                                ${i}
                                            </a>
                                        </li>
                                    </c:if>
                        ...
                        <div class="mt-2 text-muted small">
                            ...
                            ${currentPage * 10 > totalRecords ? totalRecords :
                            currentPage * 10}
